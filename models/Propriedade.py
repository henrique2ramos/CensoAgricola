...
class PropriedadeSchema(Schema):
    ...
    codigo_municipio = fields.Str(
        validate=validate.Length(equal=7), allow_none=True)
    codigo_uf = fields.Str(validate=validate.Length(equal=2), allow_none=True)
    codigo_ue = fields.Str(
        load_only=True, validate=validate.Length(equal=2), allow_none=True)
